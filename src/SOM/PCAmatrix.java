...
public class PCAmatrix {
	...
	public void info(String[] args) {
		...
		PCAmatrix.print(test);
		double[][][] SVD = PCAmatrix.singularValueDecomposition(test);
		double[][] U = SVD[0];
		double[][] S = SVD[1];
		double[][] V = SVD[2];
		...
		PCAmatrix.print(PCAmatrix.multiply(U, PCAmatrix.multiply(S, PCAmatrix.transpose(V)))); //Should be C
	}
	...
	static EigenSet eigenDecomposition(double[][] input) {
		...
		boolean done = false;
		while(!done) {
			double[][][] fact = PCAmatrix.QRFactorize(copy);
			double[][] newMat = PCAmatrix.multiply(fact[1], fact[0]); //[A_k+1] := [R_k][Q_k]
			Q = PCAmatrix.multiply(fact[0], Q);
		...
		ret.values = PCAmatrix.extractDiagonalEntries(copy); //Eigenvalues lie on diagonal
	...
	static void print(double[][] matrix) {
		for(int j = 0; j < matrix[0].length; j++) {
                    for (double[] matrix1 : matrix) {
                        double formattedValue = Double.parseDouble(String.format("%.4g%n", matrix1[j]));
